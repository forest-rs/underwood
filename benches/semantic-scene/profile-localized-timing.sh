...
for scenario in localized-prepare localized-region localized-style append; do
    for paragraphs in 64 1000; do
        : >"$results"
        sample=0
        while [ "$sample" -lt "$samples" ]; do
            "$binary" "$scenario" "$paragraphs" |
                awk -F '\t' '
                    {
        ...
        awk -v scenario="$scenario" -v paragraphs="$paragraphs" -v samples="$samples" '
            NR == 1 { minimum = $1 }
            NR == (samples + 1) / 2 { median = $1 }
            { maximum = $1 }
            END {
                printf "%s\t%d\t%d\t%d\t%d\t%d\n",
                    scenario, paragraphs, samples, minimum, median, maximum
            }
        ' "$sorted"
    done
done
