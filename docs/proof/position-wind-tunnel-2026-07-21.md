...
- **Implementation commits:** `3734c6c`, `9d2b878`, `a6e2e59`, `08f1394`
...
Twenty wind-tunnel tests pass. The load-bearing cases are:
...
- the exact `form-10k` corpus with 256 authored spans, 64 sparse anchors, and
  four IME-shaped publications ending in `日本語`, each frontier checked
  against the canonical model and private chunked/blocked candidates;
...
| `form-10k` four-publication IME sequence | 40,979 source bytes copied; 256 anchor and 1,024 span visits | 16,393 source bytes copied; 18 chunk, 4 block, and 1,024 span visits | semantic equivalence at every publication; timing gate remains a screen |
...
The `form-10k` timing screen now measures the final IME transaction through
immutable snapshot publication after preparing the first three composition
states. The recorded p95 was 15,500 ns over 200 debug-build samples. It remains
`SCREEN`, despite being below the proposed 16-ms limit, because the reference
machine and confidence protocol are not ratified.
